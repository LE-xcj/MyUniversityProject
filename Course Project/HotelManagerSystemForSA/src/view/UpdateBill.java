	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==bt1){
			...
			if(!Check.isMoney(deposit))
				new MyOptionPanel("订金的金额违法！", 0);
			else if(!Check.validDate(cDate))
				new MyOptionPanel("入住日期有误！", 0);
			else if(!Check.validDate(lDate))
				new MyOptionPanel("离店日期有误！", 0); 
			else if(!Check.dateValid(cDate, lDate))
				new MyOptionPanel("入住日期与离店日期有误！", 0);
			else if(OperaPojo.isOverlap(roomID, cDate, lDate,billNum))
				new MyOptionPanel("该时间段与其他订单的时间有冲突！", 0);
			else{
				if(0==MyOptionPanel.initConfirmPanel("确定修改？")){
					OperaPojo.updateBill(billNum, roomID, cDate, lDate, deposit);
